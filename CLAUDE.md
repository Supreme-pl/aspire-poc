...
**Do not reintroduce a `KafkaTopicEnsurer` that runs before `app.Run()`**. It was tried and caused a CI regression: admin-client retries blocked startup for up to 60 seconds, during which the HTTP listener didn't exist, and the integration test's `HttpClient` timed out posting to `app1`. Aspire's `WaitForResourceHealthyAsync` returned early because `MapDefaultEndpoints` only maps `/health` in `Development` environment (see `shared-observability/Extensions.cs`) — in Production/CI, Aspire falls back to "Running == Healthy", so the test was released while the app was still mid-ensurer. Lesson: don't put blocking infrastructure calls between `builder.Build()` and `app.Run()`.
