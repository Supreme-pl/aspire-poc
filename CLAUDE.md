...
`KafkaTopicEnsurer` in both apps now retries 30× at 2s intervals (60s total patience) and silences librdkafka's internal retry chatter via `SetLogHandler((_, _) => { })` on the admin client. The producer in app1 and consumer in app2 have the same log-handler silencer attached — prevents the 4000+ line "Connection refused" flood if any transient broker blip happens at runtime. App-level errors still surface via exception propagation; only the low-level connection retry logs are silenced.
